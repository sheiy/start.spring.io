...
public class SystemExceptionProjectContributor implements ProjectContributor {

	private final SystemException systemException;

	private final ProjectDescription projectDescription;

	public SystemExceptionProjectContributor(SystemException systemException, ProjectDescription projectDescription) {
	...
	public void contribute(Path projectRoot) throws IOException {
		...
		Path filePath = projectRoot.resolve("src/main/java/" + packageName + "/" + "exception/SystemException.java");
		Files.createDirectories(pkgPath);
		Path file = Files.createFile(filePath);
		try (PrintWriter writer = new PrintWriter(Files.newBufferedWriter(file))) {
			this.systemException.write(writer);
		}
	}
