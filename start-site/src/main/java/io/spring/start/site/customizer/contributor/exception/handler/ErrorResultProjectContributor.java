...
public class ErrorResultProjectContributor implements ProjectContributor {

	private final ErrorResult errorResult;

	private final ProjectDescription projectDescription;

	public ErrorResultProjectContributor(ErrorResult errorResult, ProjectDescription projectDescription) {
	...
	public void contribute(Path projectRoot) throws IOException {
		...
		Path filePath = projectRoot
				.resolve("src/main/java/" + packageName + "/" + "exception/handler/ErrorResult.java");
		Files.createDirectories(pkgPath);
		Path file = Files.createFile(filePath);
		try (PrintWriter writer = new PrintWriter(Files.newBufferedWriter(file))) {
			this.errorResult.write(writer);
		}
	}
